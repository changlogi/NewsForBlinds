...
def display_news_section():
    ...
    def KeyClick(e):
        pygame.mixer.music.stop()
        pygame.mixer.music.unload()
        if int(e.keycode)-48 in range(0,len(NewsSectionList)): buttons[int(e.keycode)-48].invoke()
...
def display_choose_news(news_section_no):
    ...
    def KeyClick(e):
        pygame.mixer.music.stop()
        pygame.mixer.music.unload()
        if int(e.keycode)-48 in range(0,len(news_url_list)): buttons[int(e.keycode)-48].invoke()
...
def display_news_contents(news_contents_url):
    ...
    try:
        ttsNewsEntireContent = gTTS(text=text1, lang='ko')
    except:
    ...
    label01 = Label(frame0, text=text1, justify=LEFT, wraplength=1000, width=140)
